...
class School(models.Model):
    ...
    def nearby_schools(self, radius_km=5):
        """Поиск ближайших школ в радиусе (км)"""
        ...
        for school in schools:
            try:
                ...
                if distance <= radius_km:
                    school.distance = round(distance, 2)  # Округляем до 2 знаков
    ...
    def save(self, *args, **kwargs):
        """При сохранении автоматически определяем координаты, если их нет"""
        if not self.latitude or not self.longitude:
            self.geocode_address()
        super().save(*args, **kwargs)

    def update_rating(self):
        """Обновляет средний рейтинг школы"""
...
class Review(models.Model):
    ...
    def save(self, *args, **kwargs):
        super().save(*args, **kwargs)
        # Обновляем рейтинг школы после сохранения отзыва
